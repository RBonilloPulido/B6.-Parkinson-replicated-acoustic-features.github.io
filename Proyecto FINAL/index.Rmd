...
library(kableExtra) #Nos servirá a la hora de poder personalizar las tablas, mejorando su diseño y presentación
library(effsize) # Para poder calcular el Cliff's Delta que nos servirá para ver el tamaño del efecto estimado al realizar la prueba de Mann-Whittney
```
...
datos_Parkinson <- read_delim("archive/ReplicatedAcousticFeatures-ParkinsonDatabase2019.csv", delim = ";", escape_double = FALSE, col_types = cols(HNR05 = col_number(),
...
knitr::kable(
  datos_Parkinson,
  caption = "Tabla 1: Ocho primeras filas (todas las columnas)",
  digits = 3,
  align = "c",
  format = "pipe"
  )
...
#Los resultados no mostraron diferencias estadísticamente significativas en los valores de RPDE entre sujetos con Parkinson y controles sanos (p = 0.5119).
...
#Podemos observar que las medianas de ambos grupos son similares, por lo que podríamos decir que la regularidad media del periodo de vibración no difiere de forma clara entre grupos
#Podemos observar un solapamiento claro entre el grupo sano y el grupo Parkinson, reforzando la idea de que RPDE no nos ayuda a diagnosticar por si sólo, pero puede llegar aportar información discriminativa.
#En el grupo Parkinson existe un outlier con un RPDE promedio bastante superior al resto de observaciones del mismo grupo. Esto sugiere que dicho sujeto podría presentar una dinámica vocal más inestable que otros pacientes. Su presencia refuerza
```
...
#Podemos observar que la mediana del grupo Parkinson es algo mayor que la del grupo control. Esto nos indica que los sujetos con Parkinson presentan valores centrales de PPE más altos que los sujetos sanos, lo que nos da a entender que existe algo más de irregularidad en el periodo del pitch de las personas con Parkinson.
#Podemos observar un solapamiento claro entre el grupo sano y el grupo Parkinson, reforzando la idea de que PPE no nos ayuda a diagnosticar por si sólo, pero puede llegar aportar información discriminativa.
```
...
ggplot(DP_Univ, aes(x = factor(Status), y = DFA_Media, colour = Status)) +
  ...
  labs(
    ...
    title = "Comparación valores promedio DFA entre sujetos control y Parkinson"
  )

#Podemos volver a observar que la mediana del grupo Parkinson es algo mayor que la del grupo control, dando a entender que los sujetos con PArkinson presentan valores ligramente más altos de DFA que los controles, indicando mayores correlaciones temporales persistentes en la señal vocal.
#Existe un solapamiento considerable entre ambos grupos, lo que vuelve a reforzar la idea de que estas variables no discriminan por sí solas pero aportan información que puede ser complimentaria a las demás.
#Aquí también encontramos un outlier pero esta vez presenta un DFA promedio bastante inferior al resto de observaciones del mismo grupo, que sugiere que la organización temporal de la voz del sujeto es más desestructurada.
```
...
#Podemos apreciar un ligero aumento de la dispersión global en los valores del grupo con parkinson, sugiriendo una mayor heterogeneidad interindividual en la estabilidad dinámica de la voz.
#Sin embargo, esto junto con la amplia superposición que muestran ambos grupos, no permite que se generen diferencias verdaderamente significativas a nivel grupal, impidiendo discriminar de manera clara entre grupos cuando analizamos de manera independiente.
```
...
#Observamos de nuevo un fuerte solapamiento entre ambos grupos, con algunos outliers en ambos grupos y sin un patrón de dispersión consistente a nivel grupal. Esto vuelve a reforzar la idea de no tomar en cuenta los resultados de manera independiente si no como información complimentaria a otros valores sacados del análisis de más métricas.
...
#En este caso, existe una mayor amplitud de los valores en ambos grupos, pero de nuevo nos encontramos con una amplia superposición de las distribuciones, indicando que la variabilidad global intra-sujeto no difiere de manera sistemática entre los grupos
```
